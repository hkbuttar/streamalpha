...
The consumer is deliberately never stopped once started, even at zero subscribers — restarting it on the next connection would just re-pay the "latest"-offset rebalance delay below for no benefit, and letting it idle-poll in the background is cheap. One test worth noting honestly: a version of the test suite that opened two real WebSocket connections concurrently through a single `TestClient` (`with client.websocket_connect(...) as ws1, client.websocket_connect(...) as ws2:`) hung indefinitely rather than failing cleanly — Starlette's `TestClient` doesn't appear to support two overlapping connections opened that way. Fan-out itself is proven directly against `TickBroadcaster` instead (`tests/backend/test_tick_broadcaster.py`), and confirmed for real against the actual live server above; the `TestClient` limitation is noted here rather than quietly worked around.
...
- Investigate why Starlette's `TestClient` hangs on two `websocket_connect()` calls opened concurrently in one `with` statement (see [Backend](#backend)) — not blocking, since `/ws/ticks`'s fan-out is already proven both at the `TickBroadcaster` unit level and live against a real running server, but worth a real root cause if `TestClient` ever needs to test genuine multi-client concurrency again.
