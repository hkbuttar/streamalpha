...
flowchart LR
    ...
    F -->|live| D[Render + Vercel]
```

Every stage is built, verified, and deployed (see [Current Status](#current-status)). Every stage that exists is a separate module so ingestion, consumption, and modeling can be tested and reasoned about independently.
...
**Cross-repo dependency.** `config.universe` is imported from alpha-signal-lab as a live editable install (`-e ../alpha-signal-lab` in `requirements.txt`), not copied, so the two projects can't silently drift apart on what "the universe" means. This only works locally, with both repos checked out as siblings — see [Deployment](#deployment) for how the one Render service that actually needs it (`streamalpha-ingestion`) gets it instead: a git dependency on alpha-signal-lab's public GitHub repo, not a vendored copy, keeping the same drift-proof property for the deployed case too.
...
**Deployed, following alpha-signal-lab's own established deployment convention (`render.yaml` + Vercel) exactly, extended for the piece that project doesn't need: Kafka.** The frontend is live at [https://streamalpha.vercel.app](https://streamalpha.vercel.app); the backend and its three background workers are live on Render.

- **Backend (`backend/main.py`) — Render, `render.yaml`.** One web service (`streamalpha-backend`), matching alpha-signal-lab's `render.yaml` almost exactly: same `runtime: python`, same `startCommand` shape (`uvicorn backend.main:app --host 0.0.0.0 --port $PORT`), same `fromDatabase` wiring for `DATABASE_URL`. `ALLOWED_ORIGINS` is `sync: false` (a placeholder Render prompts for at deploy time, not a real value committed to the repo) — set to the Vercel URL, same two-step chicken-and-egg deploy order alpha-signal-lab's own README describes for itself.
...
- **`streaming`'s model state survives Render redeploys.** `MODEL_STATE_PATH` writes to local disk (`streaming/model_state.py`), which isn't persistent on a Render worker by default -- a [Render Disk](https://render.com/docs/disks) is attached to `streamalpha-streaming` (`render.yaml`, mounted at `/var/data`, `MODEL_STATE_PATH=/var/data/model_state.pkl`) so redeploys no longer start every ticker's model cold.
...
- **Frontend (`frontend/`) — Vercel, zero-config.** Same as alpha-signal-lab's own React dashboard: Vercel auto-detects the Vite app, no `vercel.json` needed. `VITE_API_BASE_URL` (Vercel project env var) is set to the Render backend's URL. Live at [https://streamalpha.vercel.app](https://streamalpha.vercel.app).

- **The alpha-signal-lab dependency, resolved for deployment.** `requirements.txt`'s `-e ../alpha-signal-lab` only resolves when both repos are checked out as siblings, which is never true in Render's build environment (it clones only this repo). Of the four deployed services, only `streamalpha-ingestion` actually needs alpha-signal-lab at all (`ingestion/alpaca_stream.py` imports `config.universe.UNIVERSE`; `backend`/`streaming`/`storage` import nothing from it). Split into `requirements-base.txt` (everything all four services need, no local path) and `requirements.txt` (`-r requirements-base.txt` plus the editable local install, for local dev only, unchanged). `streamalpha-ingestion`'s `buildCommand` installs `requirements-base.txt` then `alpha-signal-lab` separately, from its public GitHub repo via a git URL, keeping the single-source-of-truth property the editable install exists for — a git dependency, not a copy that can drift, and not something to reimplement independently. This needed alpha-signal-lab's `pyproject.toml` (packaging metadata: `[build-system]`/`[project]`/`[tool.setuptools]`) pushed to GitHub for the git URL to resolve at all — confirmed live, twice: once showing the install genuinely failing before the push (a package-discovery error), and again afterward showing `pip install "alpha-signal-lab @ git+https://github.com/hkbuttar/alpha-signal-lab.git"` succeed, resolve to the real pushed commit, and `from config.universe import UNIVERSE` return all 50 tickers.
...
├── requirements.txt              # local dev: -r requirements-base.txt + -e ../alpha-signal-lab
├── requirements-base.txt          # what every Render service installs -- no local sibling-repo path
└── .env.example
```
...
- Deployed: Render (backend + 3 background workers + managed Postgres, `render.yaml`) and Vercel (frontend, live at the demo link above), following alpha-signal-lab's own established deployment convention. See [Deployment](#deployment) for the full setup.
